...
# ===== JPEG conversion (quality preserved) =====
def save_jpeg():
        ...
        for i, src in enumerate(input_files, 1):
            ...
            try:
                if not os.path.exists(src):
                    ...
                    out_path = os.path.join(output_dir.get(), f"{base}.jpg")
                ...
                errors += 1
                print(f"[Save JPEG] Error converting {src}: {e}")
...
DZ_TILE_SIZE = 256
DZ_OVERLAP = 1
DZ_IMAGE_QUALITY = 0.9  # deepzoom expects 0..1
DZ_FILTER = Image.LANCZOS  # 'antialias' equivalent/best quality

def _dz_make_creator():
    # Create a fresh ImageCreator for the worker (not per-file inside worker loop)
    return ImageCreator(
...
def create_deepzoom():
    ...
    # Auto workers selection (no user input)
    ...
    def run_parallel():
        ...
        # One ImageCreator per worker (avoid per-file re-init)
        def job(src):
            try:
